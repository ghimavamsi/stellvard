(function ($) {
  ...
  swiperslider.each(function () {
    ...
    if (carouselElement.hasClass("marquee")) {
      ...
      var space_between_val =
        $(carouselElement).data("margin") !== undefined &&
        $(carouselElement).data("margin") !== ""
          ? margin_val
          : 0;
      var swiper2 = new Swiper(".pbmit-element-viewtype-carousel-" + x, {
        spaceBetween: space_between_val,
        centeredSlides: true,
        speed: 10000,
        autoplay: {
          delay: 1,
    ...
    } else if (carouselElement.hasClass("client-marquee")) {
      var reverse_direction = $(this).data("reverse");
      if (!reverse_direction) reverse_direction = false;
      var space_between_val =
        $(carouselElement).data("margin") !== undefined &&
        $(carouselElement).data("margin") !== ""
          ? margin_val
          : 0;
      var swiper_client = new Swiper(".pbmit-element-viewtype-carousel-" + x, {
        spaceBetween: space_between_val,
        centeredSlides: false,
        speed: 2500, // even faster speed marquee
        autoplay: {
          delay: 1,
          disableOnInteraction: false,
          reverseDirection: reverse_direction,
      ...
      // Stop marquee on hover, resume on mouse leave
      carouselElement.on("mouseenter", function () {
        if (swiper_client && swiper_client.autoplay) {
          swiper_client.autoplay.stop();
        }
      });
      carouselElement.on("mouseleave", function () {
        if (swiper_client && swiper_client.autoplay) {
          swiper_client.autoplay.start();
        }
      });
    } else {
      if (autoplay2) {
  ...
  var pbmit_back_to_top = function () {
    ...
    jQuery(window).scroll(function () {
      if (jQuery(window).scrollTop() > toggleHeight) {
        ...
        jQuery(".pbmit-brochure-download").addClass("active");
      } else {
        //Removes active class to make button visible
        jQuery(".pbmit-backtotop").removeClass("active");
        jQuery(".pbmit-brochure-download").removeClass("active");
      }
    });
